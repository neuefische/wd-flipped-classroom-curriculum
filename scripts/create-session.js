...
const sessionFileNames = ['session-name.md', 'challenges-session-name.md', 'README.md'];
...
(async () => {
	try {
		...
		const sessionFileContents = await Promise.all(
			sessionFileNames.map(fileName => fs.readFile(`${sessionPath}/${fileName}`, 'utf8'))
		);
		...
		const sessionFileContentsReplaced = sessionFileContents.map(content =>
			content.replace(/Session Name/g, input.sessionName).replace(/session-name/g, slug)
		);

		// write the session files back to the session dir
		await Promise.all(
			sessionFileNames.map((fileName, index) =>
				fs.writeFile(`${sessionPath}/${fileName}`, sessionFileContentsReplaced[index])
			)
		);

		// rename the session files to the session name
		await fs.rename(`${sessionPath}/session-name.md`, `${sessionPath}/${slug}.md`);
		await fs.rename(
